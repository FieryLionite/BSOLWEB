...
function WebGL_RecreateTexture(_tex, _mipoptions) {
    ...
    // Check if there's an image and it requires loading management
    if (_tex.Image) {
        if (_tex.Image.loading === 'lazy' && !_tex.Image.complete) {
            _tex.Image.loading = 'eager'; // Force the image to load eagerly
            _tex.Image.onload = () => {
                // Recreate the texture once the image is loaded
                WebGL_RecreateTexture_Internal(_tex, _mipoptions);
            };
            _tex.Image.onerror = () => {
                console.error('Failed to load image for texture recreation');
            };
            return; // Exit and wait for the image to load
        }else if (!_tex.Image.complete)
            return;
    }
    console.log("DIRECT TO TEXT RENDER",_tex.Image)
    // Proceed directly if there's no image or it's already loaded
    WebGL_RecreateTexture_Internal(_tex, _mipoptions);
}

function WebGL_RecreateTexture_Internal(_tex, _mipoptions) {
    // Call to the original WebGL recreation function
...
function WebGL_RecreateTextureBU(_tex, _mipoptions)
{
    if (_tex.Texture)
        return;     // texture already exists
